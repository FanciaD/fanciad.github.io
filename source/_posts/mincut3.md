...
考虑到这篇比一晚上速通那篇长了5倍而且难了5倍，写这么慢也是合理的。
...
那么考虑什么样的流能满足这件事：如果一个不超过 $\delta$ 的割与当前区域相交，我们希望说，如果一侧大小不超过 $s$，那么它就不会流满。

我们看看这里面的流量是多少：

1. 从外面流进来的，这差不多是 $U\cdot \delta$ 级别。所以只要 $s\geq O(\tau\delta)$，那这部分就差不多没问题。
2. 本来就在里面的，因此我们希望有：对于任意一个权值小于 $\delta$，一侧大小小于 $s$ 的割，它一侧里面的初始流量只有 $s/3$。
...
我们想说一个小的割就不行——直观上看，能流出去的部分只有 $\delta$，因此只需要一开始放的能够超过这部分和内部流量——但很遗憾的是，内部流量可以非常大，比如总的东西的一半。

这怎么办？我们介绍 Iterative Improvements：我们一开始得到 $s=W(G)$-strong，然后每一步我们尝试在 $2s$-strong 上得到 $s$-strong。这样的话，上面第二步就只有 $2s$ 的流量。那么，我们只需要每次在一个点上 $4s$ 流量，然后开始跑流，甚至可以让 $U=s/2\delta$。此时一个流能跑满当且仅当它不在这里面——甚至能跑到 $3s$ 都当且仅当它不在割里面。这是为了保证只要不行，那么 excess 一定足够大。
...
然后看一下这堆参数：$\beta=200s/\delta,U=s/2\delta$——好像不大好。但我们可以更加放缩一下：取 $U=s/2\delta\cdot O(\tau)$，然后跑 $\tau$ 倍的那么多组东西（限制放到，比如说，$3.9s$）。然后把这 $\tau$ 倍平均掉。此时会有一些割里面的东西跑出来，但是对于每个割，跑出来的都会很少（受到 $3.9$ 影响，所以没事）这样 $\beta$ 不怎么变，但是 $U$ 那边多了一个 $\tau$ 倍，所以 $\beta/U$ 这一项解决了。
...
2. 对于每一个近似最小割（$1.01\delta$），我们可以把它近似到这些块上的一个割，满足权值增大的很少（$1+1/poly\log$ 倍），且改变的大小也很少（$poly\log$ 级别）

直观地看，我们先做上面那几步，然后除了希望 $s_0$-strong，我们还希望如下性质来满足第二条：

> 定义一个连通块 $S$ 上的一个割 $C$ 是 $(1+\epsilon)$-boundary sparse 的，如果这个割两侧在 $S$ 之中的点向 $S$ 之外的边权和都至少是 $(1+\epsilon)$ 倍这个割在 $S$ 上的权值。同时，我们只考虑 $\leq 1.01\delta$ 的割。
>
> 一个连通块是好的，如果不存在这样的割。
...
但一个流的问题不仅要有源，还要有接受流量的汇。不过我们有一个 $\epsilon$ 的差距，同时我们保证了**这个割是 s0-strong**的，所以它内部 vol 只有 $s_0$。因此考虑每个点放 $c*deg$，其中 $c$ 差不多是 $\epsilon\delta/s_0$ 级别。然后割大概只能流出去 $\delta$——

但此时我们并没有保证这里面的割就很大。因为即使整体上是 $\delta$ 的割，分到这个小块（$s_0$-strong）里面可能就很小了。

那我们先不管这件事，先做简单情况：

> 定义一个块是 Well-Connected 的，如果它的最小割很大。这里我们让它至少是 $0.1\delta$.

那此时就可以说这是合理的了。后面我们会看到，这个假设非常非常非常有用。

但还有个问题，这样可以说明，如果流到一半流不动了，那已经流满的部分确实是一个 Boundary Sparse 的割。但如果流太多（多于一半）甚至流满了呢？那就保证不了这件事了。这相当于说，我们不能往里面放太多流量。简单来说，每个点放 $c*deg$，其中 $c$ 差不多是 $\epsilon\delta/s_0$ 级别。那么假设我们就在 $vol\leq s_0$（就是一个 $s_0$-strong 的割的一侧）上面放了流量，那总流量是 $s_0$。此时如果总的 $vol\geq s_0^2/\epsilon\delta$，它就不能流满，我们就能找到一组割。
...
但有个问题——如果有多个割相交，那我们的流可能只会返回它们的交集。那这就寄了。
...
那这怎么说明我们的割在里面？不一定，但我们可以说：

> 如果这个割在里面只割了 $0.49\delta$，那它就在里面。

原因是：我们做了一个无损 Uncrossing，把前后的差拿出来，作为一个割。它的权值应该只有 $0.98\delta$。这不大可能（注意到 $\delta$ 是最小割），只能是没有变化。

事实上这个想法非常有用，后面可以证明放到块之间的割变化很小，想法就是如果一块之间只有 $\leq 0.49\delta$，那 $(1+\epsilon)$-近似的 Uncross 不能改它，不然就会有比最小割更小的块。

为此，我们再定义块上的一个割是 Good Cut，如果它满足前面那堆条件，同时满足在当前块上割边权小于 $0.49\delta$。（可以发现，这样的割一定是 Boundary Sparse 的）

那么我们可以做之前的东西。需要构造：

> 找到数量不多的大小为 $O(s_0)$ 的集合，使得任意一个
>
> 1. $\delta/10$-Well-Connected
> 2. $s_0$-strong
> 3. 割小于 $0.49\delta$。
> 4. $1+\epsilon$-Boundary-Sparse
>
> 的割都被某个集合包含。

实际上构造这个割只需要前两条。后两条只是为了保证整个做法的正确性。

##### Covering Sources

###### From Cuts

考虑如何枚举这样的集合。根据 Gabow 的做法，我们可以对图做生成树分解，然后最小割在生成树上割的比较少，然后就可以在生成树上枚举。

但众所周知这东西不能做到有权图。在上一篇里面，我们用了一整套 Expander 技术来让这东西近似正确。但如果我们只考虑（一大堆修饰词的）好的情况，那能不能更好呢？

上一篇说过，如果点数很少，那么逐项近似即可保证正确。而一般情况则是在 Expander Hierarchy 上点数很少。但此时我们有 $s_0$-strong, $\delta/10$-Well-Connected，那每个点度数至少是 $\delta/10$，所以点数至多是 $20s_0/\delta$。那如果按照 $O(\delta^3/s_0^2)$ 为单位放缩，对于这些连通块的误差就是小的，搞完（原来）还是 $O(\delta)$ 级别。然后现在边权大概是 $O((s_0/\delta)^2)$。
...
回到之前的部分，在这个连通块里面，最小割差不多是 $0.1\delta$，然后我们要考虑的割是 $\delta$，所以这是 $O(1)$ 倍近似。那么放到上面去，这个割平均在每个森林里面割了 $O(1)$ 个位置。

那么我们可以找到 $n^{O(1)}$ 个集合……但这不好。一个好的数字是 $n\times poly\log n$，或者 $n\times poly(s_0/\delta)$。

###### From Well-Connectness

唯一让事情变得不一样的条件，是我们还要求 Well-Connected。这实际上是一个非常强的条件——一个这样的集合，vol 很小($O(s_0)$)，最小割很大（$O(\delta)$，认为这两差 polylog 倍）。更进一步，我们还知道点数也很少(polylog)。那一个点为了满足这件事，它的度数必须集中到一部分点上。此时随便捏一个就不能满足条件。

我们考虑的集合大小是 $O(s_0/\delta)$ 的，因此如果想把两个集合捏在一起，它们中间至少需要一条 $O(\delta^3/s_0^2)$ 的边来使得割是 $O(\delta)$。但我们又可以只考虑 $vol\leq s_0$ 的点，这样每个点只需要考虑 $O((s_0/\delta)^3)$ 个相邻点。这可以看成每个点这么多度数的图，然后我们选的需要是一个连通块。

但直接这样枚举，我们得到的连通块数量是 $O((s_0/\delta)^{O(s_0/\delta)})$，这是不好的。可以发现这里没有用整个块还是一个小割的性质。

###### Altogether

因此我们需要把两个性质放一起。

第一个性质实际上说明我们只用考虑 $O(1)$ 个块：注意到我们还有 $vol$ 很小的条件，那么树上做一个欧拉序，按照 $vol=s_0$ 分块，然后一个割了 $k$ 次的东西在树上是 $O(k)$ 个区间，放到块上即可。

此时每个点只被 $poly(s_0/\delta)$ 个块覆盖，然后把这些造出来的块看做点，做第二步的东西。具体来说，两个 Set 连边当且仅当有点相交或者有大边权的边相连。因为每个点在 $poly(s_0/\delta)$ 个集合里面，所以边数还是不多。这样就是 $O(n\cdot (s_0/\delta)^{O(1)})$。
...
Thm. 给一堆 Terminal，我们可以对每一个找一个 $1+\epsilon$-approx 的和其它 Terminal 分开的割（Isolating Cut）。它们互不相交。

1. 类似最小割的经验，给任意一个割，可以把它缩到我们找的 Isolating Cut 里面，使得割变大的只有 $1+\epsilon$ 倍。

本质上这个东西是在说，我们每个流是来一个大源点，向每个地方连出流量。这样就可以变成单 Terminal 的 Isolating Cut。考虑这个割，直接割流量边的是在外面的部分。剩下的就是存在一个别的割使得这些流量流不出去，那这大概就是我们最开始想要的能把 Boundary Sparse 部分切出去的割。

【施工中】

最后，我们可以拿出一组 Source Set，然后对于每一个找到一个割 $C$，满足：

1. 它的 vol 不增加很多。
2. 它是 Boundary Sparse。
3. 对于本来 $S$ 中一个割 $U$，我们可以把它 Uncross 到 $C\cap U$ 里面，使得权值只增加 $1+\epsilon$ 倍。同时改变当且仅当原来在这里面割了 $\Omega(\delta)$。
...
最后把上面的东西拆成一堆不交 Set 组一起做就行了。大概就是直接贪心，因为每个 Set 相交的很少。
...
这就有一个问题——如果按照 $\delta/10$ 分，那这个代价就是 $\delta*(s_0/\delta)^2$，这太大了。如果按照更小的分，那它就不是 Well-Connected，然后就不能用之前的结论。

但如果你还记得的话，之前不能用的唯一原因是点数可能非常大，然后我们是一个 Additive 的近似。后面还有第二步把集合合并起来，但那个是可以做更小的割的——把大的边权的分界线放小 $poly\log$ 倍，从而复杂度增加一个 $poly\log$。

###### Shrinking to Smaller Sets

什么集合是小的？Well Connected（因为我们有 $s_0$-strong 的假设）是小的。

我们可以把不 Well Connected 的东西搞成一堆 Well Connected 的东西，那做法就是看到小的割就切掉。问题是，可能得到了特别多个小块。如果只有 $O(1)$ 个小块，那总的大小就很小，然后我们就赢了。怎么做呢？如果有 Good Cut，我们就可以把它删了。

再看 Boundary。因为每次割的都是 $0.1\delta$，那平均每个块会有 $0.2\delta$，加上初始边权。
...
那么可以神奇地发现，只要跑 $\log n$ 轮，那么最后就只剩下 $O(1)$ 个块。从而点数只剩下 $O(s_0/\delta)$。然后，拿出之前的东西。

###### Splitting to Better Cuts

此时我们把割的权值调到 $O(\delta\cdot (\delta/\epsilon s_0))$，这保证最后无论怎么调，分出来中间的部分都不会太影响最小割。

这样搞的话，之前把 Set 合并起来（From Well-Connectness）那里参数需要改。但这只增加 polylog 倍。

然后每一块内部套一遍之前的证明，唯一区别是取整的时候我们手动说明它的点数合理。这样每一块内部我们有一个好的 Uncrossing。加起来的话，因为中间部分很小，这就是真的好的 Uncrossing。
...
好消息是，我们分出来的块大小是很小的，实际上比 $s_0$ 大了 $s_0/\epsilon\delta$ 倍（回顾一开始的网络流构造，我们需要这么多来保证它不流满，所以最坏是切了这么多。）

这是 $\delta\cdot poly\log n$，点数从而是 $poly\log n$。因此我们可以考虑多项式算法。

#### Polynomial Uncrossing

此时啥东西都可以拿出来用了，比如直接开切。（之前只能证明的时候用，因为是 poly 的）。
...
不过有个限制：我们只考虑大小最多 $1.01\delta$ 的割。这有什么用？

直接搞好像没啥用，因为可能这部分里面最小割超级小，但如果这样呢？

##### Progress by Sparse Cuts

Rule 1: 如果图里面的最小割小于 $0.49\delta$，我们就把它割掉。
...
而且，事实上割掉这些是有理的：它们一定是 Boundary Sparse，两侧边界至少得是 $0.51\delta$，不然你就有一个更小的全局割。

那为什么我们可以这样做？

记初始 Boundary 大小为 $\partial$。我们的操作是，每次选一个大小不超过 $0.49\delta$ 的割，然后分开。我们希望证明，最后所有块的 Boundary 大小是 $\partial poly\log$。

为什么是 $0.49\delta$？因为有一个隐藏的条件：图的最小割至少是 $\delta$，所以每个块的 Boundary 大小至少是 $\delta$。

直观地看，即使每次分很小一块出来，那一边也是 $\delta$ Boundary，所以原来至少是 $0.51\delta$ Boundary。那么大的块就至少减少了 $0.02\delta$，总 Boundary 只是增加了 $0.98\delta$。

一般地，我们定义势能为 $\sum\max(0,|\partial|-\delta)$。可以证明每次势能减少 $0.02\delta$，而总 Boundary 只是增加了 $0.98\delta$。因此，最后的 Boundary 大小为 $O(\partial)$。

##### Progress of Not-too-small Components

现在，我们可以枚举可能的 $(1+\epsilon)$-boundary sparse 的割，然后考虑把它割掉。这样对吗？

此时每次相当于选一个 $[0.49\delta,1.01\delta]$ 的割割掉，然后因为两边都是 $1+\epsilon$ 倍，所以两块大小都会降低 $\epsilon$ 乘上割的权值。

如果每次割的都正好是 $1\delta$，那可以设势能为 $\sum\max(S-2\delta,0)$，用这个摊掉边界的增加。然后每步用 $\delta/\epsilon$ 的势能减少换最多 $2\delta$ 的增加……但这真的对吗？

这个分析在 $S$ 很小的时候就失效了，因为势能减少量为 $0$。确实有一个反例：现在有一块 $2\delta$ 的，对半分可以得到 $2(1-\epsilon)\delta$ 的两部分，然后继续对半分即可得到 $2(1-\epsilon)^2\delta$。这样可以搞出非常大的满二叉树递归。

但可以发现，如果只操作大小至少是 $2.33\delta$ 的块，那么每一步势能减少量还是足够少的。（考虑分成大/大，大/小和小/小的情况）。这是好的。我们可以先贪心这样做一步，然后变成如下问题：

##### Solving Small Compoments

现在每个块的边界大小只有 $2.33\delta$，然后我们还是需要做之前的东西。

如果我们一直割，如何避免之前那个指数级大小的情况？有两点观察：

1. 那样搞出来，我们考虑了指数多个割（每个割总共只有 $1.01\delta$，所以只会出现在很少的块中），但如果我们从最小割至少是 $0.49\delta$ 的东西开始，就只有 $n^4$ 个割。
2. 我们更可以从整体的角度考虑这个割：如果第一次分开后两边大小都很大，然后第二次又在左边找了个 $(1-\epsilon)\delta$ 的割……它也在右边出现！所以右边就最多 $\epsilon\delta$。根据之前的分析，小的割直接割掉就是赚的。
...
那么我们复用一下上一次的 Hierarchy，然后抄一遍上一次的证明，唯一的区别是

##### Sparsifier for Large Cuts